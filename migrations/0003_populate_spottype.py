...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for spot in orm.Spot.objects.all():
            type_names = spot.type_name.split(',')
            for tn in type_names:
                st = orm.SpotType.objects.get_or_create(name=tn)  # get_or_create returns a tuple
                spot.spottypes.add(st[0])
            spot.save()

    def backwards(self, orm):
        for spot in orm.Spot.objects.all():
            types = []
            for st in spot.spottypes.all().values():
                types.append(st['name'])
            spot.type_name = ', '.join(types)
            spot.save()
