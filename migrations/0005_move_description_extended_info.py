...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for spot in orm['spotseeker_server.Spot'].objects.all():
            if spot.description:
                orm['spotseeker_server.SpotExtendedInfo'].objects.create(key='description', value=spot.description, spot=spot)

    def backwards(self, orm):
        for desc in orm['spotseeker_server.SpotExtendedInfo'].objects.filter(key='description'):
            spot = desc.spot
            spot.description = desc.value
            spot.save()
            desc.delete()
